...
function friendly(e: unknown): string {
  ...
  if (/LackOfFundForMaxFee|insufficient funds/i.test(msg)) {
    return 'Your wallet is below the fee reserve for an AI breeding (mostly refunded). Top up at testnet-faucet.genlayer.foundation';
  }
  if (/reject|denied|4001/i.test(msg)) return 'You waved off the signature';
  if (/rate limit|429|too many/i.test(msg)) {
    return 'The lineage is busy, your cross is still being worked';
  }
  if (/network|fetch/i.test(msg)) return 'Network hiccup, check your connection';
  if (/timeout/i.test(msg)) {
    return 'The lineage is busy, your cross is still being worked';
  }
  return 'The cross did not go through. Give it another try.';
}
    ...
    try {
      ...
      if (status === 'ACCEPTED' || status === 'FINALIZED') {
        ...
        setState((s) => ({
          ...
          phase: 'error',
          liveStatus: status,
          error: 'The validators could not agree on the cross. Please try again.',
        }));
        ...
        setState((s) => ({
          ...
          phase: 'error',
          liveStatus: status,
          error: 'The cross was canceled on chain.',
        }));
      } else {
        setState((s) => ({
          ...s,
          phase: 'error',
          liveStatus: status,
          error: 'The lineage is still settling. Check the explorer for the final result.',
        }));
      }
    } catch (e) {
